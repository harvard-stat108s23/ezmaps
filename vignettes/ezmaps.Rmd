...
A cartogram distorts the geometry of geographic regions based on a variable. Regions can remain uncolored, be colored by the same variable, or be colored by a different variable. 
...
The hexbin map is a size-adjusted choropleth map that allows the user to compare how a variable differs by a region through differences in color. The user also inputs a column for labeling the different regions, since the regions are necessarily indistinguishable by shape or size. 
...
Then, if we wanted to choose seed map 4 as the base base grid used by the `hexbin()` function, we would use the `hexbin(seed)` argument. 
